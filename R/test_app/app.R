...
ui <- fluidPage(
  ...
  titlePanel("Doc Flow: Local OCR & Details Extraction (with Mistral and GPT-OSS) - TESTING"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectizeInput(
        "ocr_model",
        "OCR model (installed in Ollama)",
        choices  = c("mistral-small3.2"),   # gets replaced dynamically
        selected = "mistral-small3.2",
        options  = list(create = TRUE, placeholder = "Pick or type a model name…")
      ),
      actionButton("refresh_models", "Refresh model list", class = "btn-secondary"),
      ...
      selectizeInput(
        "extract_model",
        "Extraction model (installed in Ollama)",
        choices  = c("gpt-oss"),            # gets replaced dynamically
        selected = "gpt-oss",
        options  = list(create = TRUE, placeholder = "Pick or type a model name…")
      ),
      checkboxInput("auto_refresh_models", "Auto-refresh model list", value = TRUE),
...
server <- function(input, output, session) {
  ...
  available_models <- reactiveVal(character(0))
  models_last_refresh <- reactiveVal(NULL)

  fetch_ollama_models <- function() {
    # list_models() uses Ollama /api/tags :contentReference[oaicite:1]{index=1}
    df <- tryCatch(
      ollamar::list_models(output = "df"),
      error = function(e) NULL
    )
    if (is.null(df) || !("name" %in% names(df))) return(character(0))
    sort(unique(trimws(df$name)))
  }

  update_model_inputs <- function(models) {
    # preserve whatever the user currently has selected/typed
    cur_ocr <- isolate(input$ocr_model)
    cur_ext <- isolate(input$extract_model)

    if (is.null(cur_ocr) || !nzchar(cur_ocr)) cur_ocr <- "mistral-small3.2"
    if (is.null(cur_ext) || !nzchar(cur_ext)) cur_ext <- "gpt-oss"

    # ensure current selections remain in choices even if not installed/listed right now
    ocr_choices <- unique(c(models, cur_ocr))
    ext_choices <- unique(c(models, cur_ext))

    updateSelectizeInput(session, "ocr_model", choices = ocr_choices, selected = cur_ocr, server = TRUE)
    updateSelectizeInput(session, "extract_model", choices = ext_choices, selected = cur_ext, server = TRUE)
  }

  refresh_models_now <- function() {
    models <- fetch_ollama_models()
    available_models(models)
    models_last_refresh(Sys.time())
    if (length(models) > 0) update_model_inputs(models)
    invisible(models)
  }
  ...
  observe({
    req(isTRUE(input$auto_refresh_models))
  ...
  output$model_status <- renderUI({
    m <- available_models()
    t <- models_last_refresh()
    if (is.null(t)) return(tags$small("Model list not loaded yet."))
    if (length(m) == 0) {
      return(tags$small("No models found (or Ollama not reachable). You can still type a model name manually."))
    }
    tags$small(sprintf("Found %d installed model(s). Last refresh: %s", length(m), format(t, "%H:%M:%S")))
  })
...
observeEvent(input$run_bulk_extract, {
  ...
  observeEvent(input$load_profile, {
    ...
    if (!is.null(prof$ocr_model)) {
      # keep loaded values in the dropdown choices even if not currently listed
      mods <- available_models()
      ocr_val <- prof$ocr_model
      ext_val <- prof$extract_model

      updateSelectizeInput(
        session, "ocr_model",
        choices = unique(c(mods, ocr_val)),
        selected = ocr_val,
        server = TRUE
      )
      updateSelectizeInput(
        session, "extract_model",
        choices = unique(c(mods, ext_val)),
        selected = ext_val,
        server = TRUE
      )
